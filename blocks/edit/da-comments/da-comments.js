...
export default class DaComments extends LitElement {
  ...
  connectedCallback() {
    ...
    window.addEventListener('da-comment-highlight-click', this.handleHighlightClick);
    window.addEventListener('da-comments-orphaned', this.handleOrphanedComments);
    window.addEventListener('da-open-comments', this.handleOpenRequest);
    document.addEventListener('click', this.handleOutsideClick);
    import('../../shared/da-dialog/da-dialog.js');
    this.checkUrlForComment();

    commentService.addEventListener('change', this.handleServiceChange);
    if (commentService.initialized) this.updateThreads();
  }
  ...
  disconnectedCallback() {
    ...
    window.removeEventListener('da-comment-highlight-click', this.handleHighlightClick);
    window.removeEventListener('da-comments-orphaned', this.handleOrphanedComments);
    window.removeEventListener('da-open-comments', this.handleOpenRequest);
    document.removeEventListener('click', this.handleOutsideClick);
    commentService.removeEventListener('change', this.handleServiceChange);
  }
  ...
  updated(changedProps) {
    if (changedProps.has('_activeThreadId')) {
      setActiveThread(this._activeThreadId);
    }

    if (changedProps.has('_formState')) {
      if (this._formState?.mode === 'new') {
        this.focusCommentTextarea();
        const sel = this._formState.selection;
        if (sel && !sel.isImage) setPendingRange(sel.from, sel.to);
      } else {
        clearPendingRange();
      }
    }
  ...
  updateThreads() {
    this._threads = commentService.threads;

    if (this._activeThreadId && !this._threads.has(this._activeThreadId)) {
      this._activeThreadId = null;
    }

    this.emitCountChanged(commentService.activeCount);
    this.requestUpdate();
    this.tryPendingScroll();
  }

  tryPendingScroll() {
    if (!this._pendingScrollToComment) return;
    if (!this._threads.has(this._pendingScrollToComment)) return;
  ...
  confirmDeleteThread() {
    ...
    commentService.removeThread(threadId);
