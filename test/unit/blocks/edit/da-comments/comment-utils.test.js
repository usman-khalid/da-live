...
describe('comment-utils', () => {
  ...
  describe('POSITION_CONTEXT_RADIUS', () => {
    it('is set to 80', () => {
      expect(POSITION_CONTEXT_RADIUS).to.equal(80);
    });
  });

  describe('simpleHash', () => {
    it('returns a string', () => {
      expect(simpleHash('hello')).to.be.a('string');
    });

    it('returns consistent results for the same input', () => {
      expect(simpleHash('test')).to.equal(simpleHash('test'));
    });

    it('returns different results for different inputs', () => {
      expect(simpleHash('hello')).to.not.equal(simpleHash('world'));
    });
